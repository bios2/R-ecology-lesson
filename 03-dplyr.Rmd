...
> * Reshape a data frame from long to wide format and back with the `pivot_longer` and `pivot_wider` commands from the **`tidyr`** package.
...
to calculate the summary statistics. To make sure `summarize()` keeps the groups created with `group_by()`, we can set `.groups = "keep"`. So to compute the mean `weight` by sex:

```{r, purl = FALSE}
surveys %>%
  group_by(sex) %>%
  summarize(mean_weight = mean(weight, na.rm = TRUE), .groups = "keep")
```
...
1. the data 
2. the *names_from* column variable, whose values will become the column names
...
Let's use `pivot_wider()` to transform surveys to find the mean weight of each genus in each plot over the entire survey period. We use `filter()`,
...
Using `pivot_wider()` to create columns with names from `genus` and values from `mean_weight`, this becomes 24 observations of 11 variables, one row for each plot.
...
In this situation we are taking a group of columns in a data frame and 
reshaping them into two new variables. One variable represents the column 
...
1. the data
2. the *cols* to be reshaped into one column
3. the *names_to* variable which gives the name to the new column that will store the old column names
4. the *values_to* variable which gives the name to the new column taht will store the old column values

To recreate `surveys_gw` from `surveys_wide` we would create a names_to column 
called `genus` to consolidate the genus names, and and values_to column called 
`mean_weight` and use all columns except `plot_id` in the pivot. .
...
Note that now the `NA` genera are included in the re-pivoted format. Pivoting 
wider and then longer can be a useful way to balance out a dataset so every
...
specify what to pivot longer than what to leave alone. And if the columns are directly
...
>   summarize(n_genera = n_distinct(genus), .groups = "keep") %>%
...
>   summarize(mean_value = mean(value, na.rm=TRUE), .groups = "keep") %>%
